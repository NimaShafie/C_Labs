// Nima Shafie
// Lab 5 - Disk Scheduling 4/30/2022
...
int _CountDistance(struct arrayTypeDisk** count_array) {
	int temp_distance = 0;
...
struct arrayTypeDisk** _ScanDirection(struct arrayTypeDisk** scan_array, bool direction) {
	int temp_distance = 0;
	int temp_value = 0;
...
// given a dynamic array and a 0 (decreasing) or 1 (increasing)
// count the total distance traversed and return it
struct arrayTypeDisk** _C_Scan(struct arrayTypeDisk** c_scan_array) {
	int temp_distance = 0;
	int temp_value = 0;
	int starting_track_index = 0;
	int temp_track_index = 0;
...
void EnterDiskParameters() {
	...
	array_tracks = (type_array_disk**)malloc(seq_size * sizeof(array_tracks));
	for (int i = 0; i < seq_size; i++) {
		array_tracks[i] = (type_array_disk*)malloc(seq_size * sizeof(array_tracks));
...
void Traverse_SSTF() {
	...
	sstf_array = (type_array_disk**)malloc(seq_size * sizeof(sstf_array));
	for (int i = 0; i < seq_size; i++) {
		sstf_array[i] = (type_array_disk*)malloc(seq_size * sizeof(sstf_array));
	}
	if (sstf_array == NULL) {
		printf("\nNot enough memory exists on this system, terminating program.\n\n");
		exit(0);
...
void Traverse_Scan() {
	...
	if (seq_size > 0) {
		scan_array = (type_array_disk**)malloc(seq_size * sizeof(scan_array));
		for (int i = 0; i < seq_size; i++) {
			scan_array[i] = (type_array_disk*)malloc(seq_size * sizeof(scan_array));
...
void Traverse_C_Scan() {
	...
	// make a copy of the array to use
	if (seq_size > 0) {
		c_scan_array = (type_array_disk**)malloc(seq_size * sizeof(c_scan_array));
		for (int i = 0; i < seq_size; i++) {
			c_scan_array[i] = (type_array_disk*)malloc(seq_size * sizeof(c_scan_array));
...
// option 6
/*******************************************************************/
void FreeMemoryDiskQuitProgram() {
